...
?>
<td width="150" >
<a href="../modules/gallery/uploads/<? echo $row["image"]; ?>" rel="lightbox">
<img src="../modules/gallery/thumbs/<? echo $row["image"]; ?>" border="0" />
</a>
</td>
<td><? echo $row["title"]; ?></td> 
<td><? echo name_cat($row["in_cat"]); ?></td> 
<td><? echo basic($row["basic"]); ?></td> 
<td>
<a href="<? echo $mf; ?>view=delete&id=<? echo $row["id"]; ?>&file=<? echo $row["image"]; ?>" class="delete">
<img border='0' src='../admin/images/delete.png'/>
</a>
</td>
...
while($row = mysql_fetch_array($sql)) {
  ?>
         <tr>
	<td style="color:#015C98;"><b><? echo $row["name"]; ?></b></td>
	<td><? echo count_imgs($row["id"]); ?></td> 
	<td>
	<a href="<? echo $mf; ?>view=edit_cat&id=<? echo $row["id"]; ?>" class="add">Промяна</a>
	<a href="<? echo $mf; ?>view=del_cat&id=<? echo $row["id"]; ?>" class="delete">Изтриване</a>
	</td>
	     </tr>
  <?
...
createCrop($putq,$papka2.$rename);
//createThumbs($papka,$papka2,200);
if ($_POST["head"]==1) {
